...
当前实际加载 **31 个子模块**。
Beta 第一阶段额外加载 `th_24_shortx_ui_runtime.js` 与 `th_34_shortx_ui_lab.js`。两者运行在 `ToolHub-Beta` 隔离根目录，实验诊断写入 `diagnostics/shortx-ui/`；Stable 和 ClipHub 不读取这些实验状态。
...
- `code/th_23_screenshot_manager.js`：内部截图与公共保存副本管理界面。
