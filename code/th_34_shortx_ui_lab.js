...
(function (global) {
  ...
  proto.runShortXUiLabBasicSelfTest = function () {
    ...
    shortXUiLabWriteAtomic(this, result);
    return result;
  };
  ...
  proto.buildShortXUiLabPanelView = function () {
    ...
    var subtitle = text("Beta 隔离实验 · 当前仅验证基础运行时，不替换正式 UI", 12, onSurface2, false);
    ...
    var boundarySection = section("当前边界", "未接管悬浮球、主面板、设置页、WindowManager、IME、Canvas 或指针/OCR。异常时仅影响本实验页面。");
  ...
  proto.getSettingsHomeCategoryDefs = function (useMonetHome) {
    ...
    var child = {
      id: "shortx_ui_lab",
      title: "ShortX UI 实验室",
      desc: "Beta 基础运行时、颜色、线程和 Shape 自检",
      icon: "⚗",
      kind: "route",
      key: "shortx_ui_lab"
    };
