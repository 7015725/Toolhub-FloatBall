...
BRANCH = "fix/pickword-emoji-finalize-20260718"
CLEAN_BASE = "58f51daf73933ed3f7aabd497d6481fac2bb51e8"
...
def run_bootstrap():
    ...
    source = replace_once(source, "// @version 1.0.14\n", "// @version 1.0.15\n", "module-version")
    source = replace_once(
        source,
        '            units: [],\n            unitStarts: [],\n',
        '            units: [],\n',
...
'''
    ...
    emoji_verify = root / "scripts" / "verify_pickword_emoji_grapheme.py"
    verify = emoji_verify.read_text(encoding="utf-8")
    verify = replace_once(verify, '// @version 1.0.14\\n', '// @version 1.0.15\\n', "emoji-verifier-version")
    verify = replace_once(
        verify,
    ...
    emoji_verify.write_text(verify, encoding="utf-8")

    for verifier_name in ("verify_pickword_long_click_api34.py", "verify_pickword_unified_cleanup.py"):
        verifier_path = root / "scripts" / verifier_name
        verifier_text = verifier_path.read_text(encoding="utf-8")
        verifier_text = verifier_text.replace("1.0.14", "1.0.15")
        verifier_path.write_text(verifier_text, encoding="utf-8")

    old_record_path = root / "updates" / "records" / "fix-pickword-emoji-grapheme.json"
    ...
    new_record_path = root / "updates" / "records" / "optimize-pickword-emoji-drag-count.json"
    new_record = {
        "schema": 1,
        "id": "optimize-pickword-emoji-drag-count",
        "type": "optimize",
        "title": "优化拾字 Emoji 拖选计数热路径",
        "details": [
            "拖选计数改为只扫描当前拖选范围内的字素，不再每40毫秒全量遍历整篇文本",
            "使用拖选开始时的字素计数作为稳定基线，分别计算添加与移除范围后的可见字符数",
            "移除逐字素闭包分配和未读取的unitStarts状态，降低8000字长文本拖选期间的system_server开销",
            "保持Emoji字素分段、UTF-16选区范围、翻译替换、撤销、钉屏、放大镜和生命周期边界不变",
        ],
        "manifestVersion": 0,
    }
    new_record_path.write_text(json.dumps(new_record, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    clean_verifier = run_git(root, ["show", CLEAN_BASE + ":scripts/verify_changed_module_versions.py"], capture=True).stdout
    (root / "scripts" / "verify_changed_module_versions.py").write_bytes(clean_verifier)
    (root / "scripts" / "pickword_emoji_perf_bootstrap_tmp.py").unlink(missing_ok=True)
    ...
    for check in (
        ...
        subprocess.run(["python3", str(root / "scripts" / check)], cwd=str(root), check=True)
