...
require('function checkLocalTrustedModuleSet()', "local trusted module preflight")
require('var trustedHash = getTrustedSha(relPath);', "persisted trusted hash read")
require('var actualHash = sha256File(file.getAbsolutePath());', "local module hash read")
require('!hashesEqual(actualHash, trustedHash)', "strict local trusted hash comparison")
require('ret.localTrustedFallback = true;', "observable local trusted fallback")
require('Trusted manifest unavailable at boot; verified locally trusted module set', "local fallback log")
require('trusted manifest missing; local trusted module set invalid:', "fail-closed local fallback")
require('回退通道模块清单自检失败:', "rollback module preflight")
