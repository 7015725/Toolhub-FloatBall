...
require(SHELL, 'this.execShellViaShortxAction(cmdB64, requestedRoot)', "root/non-root action path uses original command")
require(SHELL, 'ShellCommand 底层使用已附着的 ShellService', "root action ShellService rationale")
require(SHELL, 'fallback=none', "executed action failure is not replayed")
forbid(SHELL, 'shell root requested fallback=broadcast_bridge', "root must not skip action")
forbid(SHELL, 'su 0 sh -c', "root action must not nest su wrapper")
