...
new_rows = '''  var rows = Math.max(1, Math.ceil(items.length / spec.cols));
  ...
  var pageContext = null;'''
main = replace_once(main, old_rows, new_rows, "grid dimensions")
...
# ---------------------------------------------------------------------------
# th_15_extra.js：主面板使用网格反推的 EXACTLY 宽高。
# ---------------------------------------------------------------------------
...
show = replace_once(show, old_measure, new_measure, "exact main measurement")
...
for name in (
    ...
    source = read(name)
    if "1.4.1" not in source:
        raise SystemExit("FAIL version anchor missing: " + name)
    write(name, source.replace("1.4.1", "1.5.0"))
...
for index, line in enumerate(readme_lines):
    if line.startswith("- 主按钮面板采用可配置自适应网格"):
        readme_lines[index] = (
            ...
            "拖动排序、分页吸附、新增和编辑按钮直接保存、单页隐藏分页圆点和关闭闪烁。"
        )
        break
else:
...
if "网格决定面板宽高" not in structure:
    ...
    if not line.endswith("|"):
        raise SystemExit("FAIL STRUCTURE row")
    line = line[:-1].rstrip() + "；网格决定面板宽高 |"
    structure = structure[:pos] + line + structure[line_end:]
write("STRUCTURE.md", structure)

print("OK applied formal grid-sized main panel patch")
